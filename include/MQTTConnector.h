...
void messageReceived(String &topic, String &payload) {
  ...
  else
  {
    ...
    if(payload.substring(26,27)=="t")
    {
      LedStatusChauf = true;
    }
    else
    {
      LedStatusChauf = false;
    }
  }



  if(payload.substring(11,12)=="H")
    {
    ...
    Serial.println(LedStatusClim);
    Serial.print("Chauffage : ");
    Serial.println(LedStatusChauf);
}
...
void sendPayloadString(String Chaine)
{
  char attributes[200];
  // Payload += "}";
  Chaine.toCharArray(attributes, 200);
  ClientMQTT.publish("v1/devices/me/telemetry", attributes);

  Serial.print("Chaine -> ");
  Serial.println(Chaine);
  Serial.print("Chaine length -> ");
  Serial.println(Chaine.length());

  // Payload="{";
}

void appendPayloadString(String Name)
{
